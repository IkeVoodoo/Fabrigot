    ...
    private synchronized boolean register(@NotNull String label, @NotNull Command command, boolean isAlias, @NotNull String fallbackPrefix) {
        ...
        Data.FABRIGOT_SERVER.getCommandRegister().registerSpigot(
                ...
                command instanceof PluginCommand pluginCommand
                        ? (sender, command1, alias, args) -> pluginCommand.getTabCompleter() == null
                                                                    ? null
                                                                    : pluginCommand.getTabCompleter().onTabComplete(sender, command1, alias, args)
                        : (sender, command1, alias, args) -> null);
